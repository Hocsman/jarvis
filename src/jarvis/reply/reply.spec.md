...
4. Conversation Memory Enrichment (gated)
   ...
   - Extract search parameters via `extract_search_params_for_memory(query, base_url, router_model, ..., context_hint=...)`.
     ...
     - **Failure is not emptiness**: the extractor returns `None` when it could not run at all (no model configured, nothing usable after both attempts, an error). That is a different answer from `{"keywords": []}`, which is the extractor reading the query and deciding it needs no memory search. On `None` the engine prints `⚠️ 🔍 Memory search: keywords could not be read this turn` and never writes the result to the hot-window cache: that cache has no age-based expiry, so caching one unavailable minute would keep the same question memory-blind for the rest of the conversation.
...
- **Failure is not emptiness**: when a distil call cannot be made (backend down, timeout, no response) `digest_memory_for_query` raises `MemoryDigestError` instead of returning an empty note. The caller then leaves `conversation_context` and `graph_context` untouched — memory that could not be read is memory that is kept, never memory that is discarded. An empty digest means the distil read the snippets and judged none of them relevant, and only then are the raw blocks dropped. A batch that fails aborts the whole digest for that turn: a note built from the subset of batches that happened to answer is itself a silent loss.
- **User-facing logging**: prints `🧩 Memory digest: N chars — "preview"` when relevant, `🧩 Memory digest: no directly-relevant past memory` when the distil returned NONE, or `⚠️ 🧩 Memory digest: unavailable this turn — keeping raw memory` when it could not run. The third line is printed rather than only logged so the fallback is visible without `JARVIS_VOICE_DEBUG`. Debug logs record raw→digest size and batch counts under the `memory` category.
