...
**And the pin decides the endpoint, not just the name.** Called without a model, `get_llm_backend` chooses the destination from `llm_provider` alone, so until `get_private_backend` existed a pinned local tag was sent *to the cloud* — verified, the request reached `https://openrouter.ai/api/v1` carrying his sentence, and the tag would have been rejected there anyway. The model-aware form (a bare tag passed as `model` on a remote provider routes to local Ollama) exists for auxiliary passes; a privacy pin must not depend on the caller remembering to pass it. With nothing pinned there is nothing to honour and the ordinary provider applies: a user who never asked for this is not quietly moved off the one he chose. The same helper serves the routine extractor, the goal judge and the journal reader, which make the same promise for the same reason.
