...
- `warm_up(model, keep_alive="30m")` first verifies the endpoint is actually an Ollama server via `GET /api/version`, then issues `POST /api/generate` with an empty prompt and the requested `keep_alive`; the model stays resident for that duration after the call.
