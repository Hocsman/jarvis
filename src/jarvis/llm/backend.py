...
# (llama.cpp, vLLM, etc.) reject unknown fields with 400 or 422.
...
def strip_nonstandard_message_fields(
    ...
    serialised because strict servers (e.g. llama.cpp, vLLM)
    reject unknown fields with a 400 or 422 error.

    Each message retains only the fields allowed for its ``role``;
    messages whose role is not in the known set are returned as-is
    so future roles (``developer``, ``function``) are not silently dropped.
    """
