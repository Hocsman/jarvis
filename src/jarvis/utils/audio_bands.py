"""Thread-safe audio ring buffer + FFT band analyser.

Originally lived at ``desktop_app/orb/audio_bus.py`` (Phase 1). Moved
to ``jarvis.utils`` in Phase 2C so the daemon process can use the same
analyser without importing ``desktop_app`` — the daemon → desktop_app
direction is forbidden by the project's layering rule (the desktop is
a *consumer* of the daemon, not the other way around).

``desktop_app.orb.audio_bus`` re-exports the same names from this
module for backwards compatibility; existing imports keep working
unchanged.
