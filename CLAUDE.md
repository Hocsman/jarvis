...
| `src/jarvis/routines/routines.spec.md` | What runs unattended at a fixed hour: recurrence rules, the envelope in `yuba/routines.md`, the spoken-schedule extractor, the routine's catalogue in the engine, the four re-checks at the gate | Every default leans shut; an empty list is empty, not "all"; a routine can only ever read, and only inside its envelope; the user's profile stays home unless the block asks for it |
