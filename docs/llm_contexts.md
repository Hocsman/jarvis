...
- **Inputs**:
  ...
  - Digested memory enrichment (optional, see #4). Graph recall is framed as things the assistant looked up, describing the world rather than the user, and yielding to the core block above when the two disagree. It carried the core block's wording until the graph stopped holding anything about the user, at which point the label was describing the wrong thing.
...
- **File**: [src/jarvis/reply/enrichment.py](src/jarvis/reply/enrichment.py) — `extract_search_params_for_memory()` (~line 37).
...
- **System prompt**: inline at [enrichment.py:66-93](src/jarvis/reply/enrichment.py:66). Byte-static: no hint block, no timestamp: so the system prompt is identical across every extractor call and stays cacheable; the per-call hint / UTC anchor rides at the end of the user content.
...
- **Model / gating**: `cfg.llm_chat_model` via `get_auxiliary_backend(cfg, model)` (the enrichment shim). Gated by `memory_digest_enabled`; the auto-on path reads the same chat model so model-size detection follows the active provider. The chat model is namespaced on a remote endpoint, so the digest stays on the configured provider; a bare tag would route to local Ollama.
...
- **Model / gating**: `cfg.llm_chat_model` via `get_auxiliary_backend(cfg, model)` (the enrichment shim). Gated by `tool_result_digest_enabled` — auto-on for SMALL via `detect_model_size(cfg.llm_chat_model)`. The chat model is namespaced on a remote endpoint, so the digest stays on the configured provider; a bare tag would route to local Ollama.
...
- **File**: [src/jarvis/tools/selection.py](src/jarvis/tools/selection.py) — `_select_llm()` (~line 275).
...
- **Output**: comma-separated tool names or `none`. Capped at `_LLM_MAX_SELECTED` (5). Always-included tools (`stop`, `remember`, `forget`, `toolSearchTool`) are unioned in regardless — an explicit "remember that…" or "forget that" must not be lost to a router miss. The "router came back empty" fallback looks past the mandatory names at what actually matched, so adding one never changes when the fallback fires.
...
- **Model**: `cfg.llm_chat_model` via `get_auxiliary_backend(cfg, model)` (the graph-ops shim). The chat model is namespaced on a remote endpoint, so extraction stays on the configured provider; a bare tag would route to local Ollama.
...
- **Model**: same `picker_model` chain as #11 (small router model when configured, falls back to `cfg.llm_chat_model`), dispatched via `get_auxiliary_backend(cfg, model)` like #11. Temperature 0 — the task is rule-following classification.
...
- **Weather** ([src/jarvis/tools/builtin/weather.py](src/jarvis/tools/builtin/weather.py), ~line 60) — dispatched via `get_auxiliary_backend(cfg, model)`: a bare local tag on a remote OpenAI-compatible endpoint routes to local Ollama. Place extractor model resolution: `tool_router_model → intent_judge_model → cfg.llm_chat_model` so small/warm models handle the parse without paging in the chat model. Parses location/time/unit from the query.
...
- **Model**: `reminder_model → tool_router_model → intent_judge_model → cfg.llm_chat_model`. Unlike the auxiliary models, `reminder_model` is deliberately **not** passed through `_cloud_safe_model` (which replaces a bare pin with the chat model only when `llm_base_url` is a remote endpoint, and prints the substitution): this prompt carries the user's own sentence about their own life, and a pin is the only way to keep it off the network — and it now does that, via `get_private_backend`, which routes an explicitly pinned model to the local backend. Called without a model, `get_llm_backend` picks the endpoint from `llm_provider` alone, so before that a pinned local tag was sent to the cloud carrying his sentence. Contexts #17, #18 and #19 share the helper and the promise.
...
- **Limits**: `appris_timeout_sec` (30 s, clamped 5–120), `temperature=0.0` for #10's reason — this is rule-following classification, and a default sampling temperature makes small models drift off a banned-forms list. At most `appris_max_propositions` survivors.
