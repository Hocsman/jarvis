...
- **Output**: comma-separated tool names or `none`. Capped at `_LLM_MAX_SELECTED` (5). Always-included tools (`stop`, `remember`, `toolSearchTool`) are unioned in regardless — an explicit "remember that…" must not be lost to a router miss. The "router came back empty" fallback looks past the mandatory names at what actually matched, so adding one never changes when the fallback fires.
