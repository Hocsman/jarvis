"""Tests for process-wide serialisation of PortAudio stream lifecycle calls.

PortAudio documents stream open and close as not thread-safe. Unserialised
lifecycle calls across threads cause hard process aborts on Windows.

The defence is behavioural and testable on any platform:
1. Every stream lifecycle call goes through the process-wide portaudio_lock,
   so two threads can never be inside PortAudio lifecycle code at the same time.
2. The dictation engine never opens streams on the pynput hook thread. The hotkey
   callback returns immediately and stream work happens on a worker thread.

These tests install a fake sounddevice whose lifecycle entry points record
concurrency and calling thread, then drive the real engine code.
"""
...
def _drain_worker_threads(baseline):
    """Join threads spawned during a test so a late worker cannot run against
...
def test_hotkey_press_does_not_open_stream_on_calling_thread(monkeypatch):
    """The calling thread must return without touching PortAudio."""
...
def test_stop_before_stream_opens_still_closes_stream(monkeypatch):
    """Press and release faster than the stream opens: no leaked live stream."""
    probe = _ConcurrencyProbe()
...
def test_stale_worker_from_superseded_press_never_stores_its_stream(monkeypatch):
    ...
    The first stale worker must not attach its stream to the second session.
    """
    probe = _ConcurrencyProbe()
    open_gate = threading.Event()
    engine = _make_engine(monkeypatch, probe, open_gate=open_gate)
    ...
    closed = [c for c in probe.calls if c[0] == "close"]
    assert len(opened) == 2
    assert len(closed) >= 1, "stale worker stream was never closed"
...
def test_serialised_stream_closes_on_start_failure():
    """If stream.start() raises inside _serialised_stream, the stream must still be closed."""
    from jarvis.listening.listener import _serialised_stream
...
def test_dictation_end_callback_suppressed_when_superseded_by_newer_session(monkeypatch):
    """When a transcribing session finishes after a newer session has started recording,
    _on_dictation_end must not be called, preventing unpausing the voice listener."""
    probe = _ConcurrencyProbe()
    engine = _make_engine(monkeypatch, probe)

    end_called = []
    engine._on_dictation_end = lambda: end_called.append(True)

    # Session 1: simulate finishing recording and handing off to transcribe
    token_session_1 = 1
    engine._session = 1
    engine._recording = False

    # Meanwhile, Session 2 starts recording
    engine._session = 2
    engine._recording = True

    # When Session 1 finishes transcribing
    engine._transcribe_and_paste([], token_session_1)

    assert end_called == [], "on_dictation_end was fired for a superseded session while session 2 is active"
