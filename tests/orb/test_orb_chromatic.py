"""Chromatic-aberration rim tests.

The chromatic-aberration helper draws two extra coloured ellipses
(red + blue) at a small horizontal offset when the orb is in an
...
3. The painter's previous composition mode is restored after the
   helper exits: subsequent ``paintEvent`` draws (particles, inner
   highlight) assume CompositionMode_SourceOver.

4. The rims sit two pixels either side of the body.
"""
...
class TestChromaticAberrationCompositionMode:
    ...
    def test_composition_mode_restored_on_exit(self, _qapp) -> None:
        ...
        try:
            ...
            widget._draw_chromatic_aberration(
                painter, cx=160.0, cy=160.0, r=80.0,
                snap=_make_snapshot(OrbState.THINKING),
            )
...
class TestChromaticAberrationOffset:
    """The rims sit two pixels either side of the body: the red one at
    cx - 2, the blue one at cx + 2."""

    @pytest.mark.unit
    def test_rims_sit_two_pixels_either_side(self, _qapp) -> None:
        from desktop_app.orb.orb_widget import OrbWidget
        from desktop_app.orb.state_controller import OrbState
