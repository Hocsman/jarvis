...
@pytest.mark.unit
def test_small_model_starts_in_text_tools_mode_and_memoises_across_turns(
    mock_config, db, dialogue_memory
):
    """Small models (e.g. gemma4:e2b) start in text-tools mode immediately.
    The memoised context line must sit before the syntax guidance on Turn 1,
    and remain byte-identical without duplication on Turn 2."""
    from jarvis.reply import engine as engine_mod
            ...
            return {
                "message": {
                    "content": (
                        ...
                        '"function": {"name": "webSearch", "arguments": "{\\"search_query\\": \\"london weather\\"}"}}]'
                    )
                }
            }
        return {"message": {"content": "The weather is sunny."}}

    with _patched_reply(engine_mod, fake_chat, return_value="Monday, London"), \
         patch.object(
             engine_mod,
             "run_tool_with_retries",
             return_value=ToolExecutionResult(
                 success=True, reply_text="sunny 20C", error_message=None
             ),
         ):
        engine_mod.run_reply_engine(
            db=db, cfg=mock_config, tts=None,
            text="weather in london", dialogue_memory=dialogue_memory,
        )

    assert len(captured) == 2, "turn 1 (tool call) + turn 2 (final answer)"
    sys_turn1 = captured[0]
    assert "Exact tool-call syntax" in sys_turn1
    assert "[Context: Monday, London]" in sys_turn1
    assert sys_turn1.count("[Context: Monday, London]") == 1
    assert sys_turn1.index("[Context:") < sys_turn1.index("Exact tool-call syntax")
    sys_turn2 = captured[1]
    assert sys_turn2 == sys_turn1, (
        "system message must be byte-identical across turns in text-tools mode"
    )


@pytest.mark.unit
def test_tool_search_widening_reinjects_context_cleanly(
    mock_config, db, dialogue_memory
):
    """When toolSearchTool widens the allow-list in text-tools mode, the
    first system message is rebuilt with the new tools description, and the
    memoised context must be re-injected cleanly before the syntax guidance."""
    from jarvis.reply import engine as engine_mod
        ...
        if len(captured) == 1:
            return {
                "message": {
                    "content": (
                        'tool_calls: [{"id": "call_1", "type": "function", '
                        '"function": {"name": "toolSearchTool", "arguments": "{\\"query\\": \\"weather\\"}"}}]'
                    )
                }
            }
        return {"message": {"content": "Found the tool."}}
    ...
    with _patched_reply(engine_mod, fake_chat, return_value="Monday, London"), \
         patch.object(
             ...
             return_value=ToolExecutionResult(
                 success=True,
                 reply_text="Discovered 1 tool(s):\n- getTime: Get time and date",
                 error_message=None,
             ),
         ):
        engine_mod.run_reply_engine(
            db=db, cfg=mock_config, tts=None,
            text="check time", dialogue_memory=dialogue_memory,
        )

    assert len(captured) == 2
    sys_turn2 = captured[1]
    assert "getTime" in sys_turn2, "widened tool must appear in system prompt"
    assert "[Context: Monday, London]" in sys_turn2
    assert sys_turn2.count("[Context: Monday, London]") == 1
    assert sys_turn2.index("[Context:") < sys_turn2.index("Exact tool-call syntax"), (
        "re-injected context block must sit before tool-call syntax guidance"
    )
